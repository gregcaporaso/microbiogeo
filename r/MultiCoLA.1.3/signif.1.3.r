...
#colnames(result3)<-c("whole.sig","ENV1.sig","ENV2.sig","ENV3.sig","ENV4.sig")
	...
	if (length(SPE)<=nrow(SPE)){
		...
		ENV1.sig<-permutest.cca(rda(Q2~ENV1+Condition(ENV2)+Condition(ENV3)+Condition(ENV4)),permutation=1000,model="full")
		...
		ENV2.sig<-permutest.cca(rda(Q2~ENV2+Condition(ENV1)+Condition(ENV3)+Condition(ENV4)),permutation=1000,model="full")
		...
		ENV3.sig<-permutest.cca(rda(Q2~ENV3+Condition(ENV2)+Condition(ENV1)+Condition(ENV4)),permutation=1000,model="full")
		...
		ENV4.sig<-permutest.cca(rda(Q2~ENV4+Condition(ENV2)+Condition(ENV3)+Condition(ENV1)),permutation=1000,model="full")
