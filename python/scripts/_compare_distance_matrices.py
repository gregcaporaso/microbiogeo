...
script_info = {}
script_info['brief_description'] = "Script for computing Mantel correlations \
                                    between sets of distance matrices"
script_info['script_description'] = """The script compare_distance_matrices.py \
compares two or more distance/dissimilarity matrices.

Mantel will test the correlation between two matrices. The data \
often represents the "distance" between objects.

partial Mantel is a first-order correlation analysis that \
utilizes three distance(dis-similarity) matrices. This builds \
on the simple Mantel which is a procedure that tests the \
hypothesis that distances between the objects within a given \
matrix are linearly independent of the distances withing those \
same objects in a separate matrix. It builds on the simple Mantel \
by adding a third "control" matrix.l

Mantel correlogram is a plot of distance classes versus Mantel \
statistics. Briefly, an ecological distance matrix (e.g. UniFrac \
distance matrix) and a second distance matrix (e.g. spatial distances, \
pH distances, etc.) are provided to the script. The second distance \
matrix has its distances split into a number of distance classes \
(the number of classes is determined by Sturge's rule). A Mantel \
test is run over these distance classes versus the ecological distance \
matrix. The Mantel statistics obtained from each of these tests is \
then plotted in a correlogram. A filled-in point on the plot indicates \
that the Mantel statistic was statistically significant \
(you may provide what alpha to use).
"""
script_info['script_usage'] = []
script_info['script_usage'].append(("Partial Mantel",
"Perform a partial Mantel test on two distance matrices, "
"using a third matrix as a control. Also run 100 permutations.",
...
"Perform Mantel test on all pairs of four distance matrices, "
"including 1000 Monte Carlo iterations. Write the output to mantel_out.txt.",
"%prog --method mantel "
...
script_info['output_description']= """Mantel: One file is created \
containing the Mantel 'r' statistic and p-value.

partial Mantel: One file is created in the output directory, which \
contains the partial Mantel statistic and p-value.

Mantel Correlogram: Two files are created in the output directory: \
a text file containing information about the distance classes, their \
associated Mantel statistics and p-values, etc. and an image of the \
correlogram plot.
"""
script_info['required_options'] = [\
 # All methods use these
make_option('--method',
    help='Matrix Correletion method to be executed. Valid options: '
...
script_info['optional_options'] = [
 ...
 make_option('-n','--num_permutations',
    help='the number of iterations to perform',default=100,type='int'),
...
make_option('-t','--tail_type',
    help='the type of tailed test to perform(1 or 2 tailed [default:%default]',
    default='two sided'),
...
comment_mantel_pmantel = """# Number of entries refers to the number \
of rows (or cols)
...
def main():
    ...
    if opts.method == 'mantel':
        ...
        #this is where the heading information is added, it accounts
        #for the spacing between file names for the first two elements
        #DM1 and DM2, but it doesn't fix the spacing between the
        ...
        for i,fp1 in enumerate(input_dm_fps):
            for fp2 in input_dm_fps[i+1:]:
                #this is a relatively benign looking line of code, but
                #a lot is going on in the background that isn't
                #being seen. For starters it's parsing distance matrice
                #files and returning the infomration as a list in an
                #array, then it has it made compatable and returned
                ...
                #This takes in a distance matrix object for the
                #dm1 and dm2 of Mantel
                m = Mantel(DistanceMatrix(dm1, dm1_labels, dm1_labels),
                ...
                third_word_spaces_needed = \
                    len("Number of entries") - len(dm1_labels)
                third_word_spaces = ""
        ...
        res_file = open(path.join(opts.output_dir,
                        "mantel_partial_results.txt"), 'w')
        ...
        for i,fp1 in enumerate(input_dm_fps):
            for fp2 in input_dm_fps[i+1:]:
                ...
                (dm1_labels, dm1), (cdm_labels, cdm) = \
                    make_compatible_distance_matrices(
                    parse_distmat(open(fp1, 'U')),
                    parse_distmat(open(control_dm_fp, 'U')),
                ...
                res_file.write('\n%s\t%s\t%f\t%f' %
                                (fp1,fp2,res['mantel_r'], res['mantel_p']))
        res_file.close()
        ...
        results_f.write('\nDM1\tDM2\tClass index\tNum dists\tMantel '
                        'stat\tp-val\tp-val (Bonferroni corrected)\n')
        ...
        for i,fp1 in enumerate(input_dm_fps):
            for fp2 in input_dm_fps[i+1:]:
                ...
                (dm1_labels, dm1), (dm2_labels, dm2) = \
                    ...
                    parse_distmat(open(fp2, 'U')), lookup=sample_id_map)
                if len(dm1_labels) < 3:
                    option_parser.error("The distance matrices were not large "
                                        "enough after filtering them to "
                                        "include only samples that were in "
                                        "both matrices. The minimum required "
                                        "size to compute a Mantel "
                                        "correlogram is 3x3.")
